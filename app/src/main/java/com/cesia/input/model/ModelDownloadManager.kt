...
class ModelDownloadManager(private val context: Context) {
    ...
    /**
     * 下载桥梁插件（native-bridge.so）
     * 桥梁是本地语音识别和 AI 润色的 native 引擎
     *
     * @param force 是否强制重新下载
     * @param onProgress 进度回调（0-100）
     * @return Result<File>
     */
    suspend fun downloadBridge(
        force: Boolean = false,
        onProgress: ((Int) -> Unit)? = null
    ): Result<File> = withContext(Dispatchers.IO) {
        try {
            val bridgeDir = ModelRegistry.Bridge.getInstallDir(context)
                ?: return@withContext Result.failure(Exception("无法获取 native 库目录"))

            val destFile = File(bridgeDir, ModelRegistry.Bridge.FILE_NAME)

            // 已存在且不强制重下
            if (destFile.exists() && !force) {
                Log.i(TAG, "Bridge already exists: ${destFile.absolutePath}")
                onProgress?.invoke(100)
                return@withContext Result.success(destFile)
            }

            val url = ModelRegistry.Bridge.getDownloadUrl()
            Log.i(TAG, "Downloading bridge from $url")
            ...
            if (!response.isSuccessful) {
                return@withContext Result.failure(
                    Exception("HTTP ${response.code} for $url")
                )
            }
            ...
            val totalBytes = body.contentLength().takeIf { it > 0 }
                ?: ModelRegistry.Bridge.SIZE_BYTES

            // 使用临时文件
            val tempFile = File(bridgeDir, "${ModelRegistry.Bridge.FILE_NAME}.tmp")
            ...
            if (tempFile.renameTo(destFile)) {
                ...
                Log.i(TAG, "Bridge download complete: ${destFile.absolutePath} (${destFile.length()} bytes)")
                Result.success(destFile)
            } else {
                tempFile.delete()
                Result.failure(Exception("Failed to rename temp file"))
            ...
            Log.e(TAG, "Bridge download failed", e)
            Result.failure(e)
        }
    }
     ...
     * 检查桥梁是否已安装
     */
    fun isBridgeInstalled(): Boolean = ModelRegistry.Bridge.isInstalled(context)

    /**
     * 检查桥梁是否可加载
     */
    fun canLoadBridge(): Boolean = ModelRegistry.Bridge.tryLoad()

    /**
     * 删除桥梁插件
     */
    fun deleteBridge(): Boolean {
        val dir = ModelRegistry.Bridge.getInstallDir(context) ?: return false
        val file = File(dir, ModelRegistry.Bridge.FILE_NAME)
